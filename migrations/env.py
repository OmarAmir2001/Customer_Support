...
def include_object(object, name, type_, reflected, compare_to):
    """Keep LangGraph's own tables out of autogenerate.

    AsyncPostgresSaver.setup() creates and owns `checkpoints`,
    `checkpoint_writes` and `checkpoint_blobs`. They are not in
    SQLAlchemyBase.metadata, so without this filter
    `alembic revision --autogenerate` writes DROP TABLE for all three.
    """
