...
from ..VectorDBEnum import (PgVectorTableSchemeEnums,
...
class PgVectorProvider(VectorDBInterface):
    ...
    async def connect(self):
        async with self.db_client as session:
            async with session.begin():
                await session.execute(sql_text(f"CREATE SCHEMA IF NOT EXISTS vector"))
    ...
    async def list_all_collections(self)-> List:
         ...
         async with self.db_client as session:
                     async with session.begin():
                           list_tbl= sql_text("SELECT tablename FROM pg_tables WHERE LIKE :prefix")
                           results = await session.execute(list_tbl,{"prefix":self.pg_vector_table_prefix})
        ...
        async with self.db_client() as session:
            async with session.begin():
                check_sql = sql_text(f""" 
                                    ...
                                    """)
                results = await session.execute(check_sql, {"index_name": index_name,
