...
class AssetModel(BaseDataModel):
        ...
        async with self.db_client() as session:
            async with session.begin():
                stmt = select(Asset).where(
                    Asset.asset_project_id == asset_project_id, Asset.asset_type == asset_type
                )
    ...
    async def get_asset_by_asset_id(self, asset_project_id: int, asset_id: int):
        ...
        async with self.db_client() as session:
            async with session.begin():
                stmt = select(Asset).where(
                    Asset.asset_project_id == asset_project_id, Asset.asset_id == asset_id
                )
                result = await session.execute(stmt)
    ...
    async def get_asset_by_id(self, asset_project_id: str, asset_name: str):
        async with self.db_client() as session:
            async with session.begin():
                stmt = select(Asset).where(
                    Asset.asset_project_id == asset_project_id, Asset.asset_name == asset_name
                )
                result = await session.execute(stmt)
                record = result.scalar_one_or_none()
            return record
