...
async def ingest_data(
    ...
    return JSONResponse(
        content={
            "signal": ResponseSignal.FILE_INGESTION_SUCCESS.value,
    ...
    for asset_id, file_id in project_files_ids.items():
        if process_controller.get_file_extension(file_id=file_id) == ProcessingEnum.JSON.value:
            ...
            file_chunks = process_controller.process_file_content(
                file_content=file_content, file_id=file_id, chunk_size=chunk_size, overlap=overlap
            )
...
async def search_knowledge_base(request: Request, project_id: int, search_request: SearchRequest):
    ...
    nlp_controller = RetrievalController(
        vectordb_client=request.app.state.vectordb_client,
        embedding_client=request.app.state.embedding_client,
        collection_name=collection_name,
    )

    results = await nlp_controller.retrieve(
